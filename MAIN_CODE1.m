...
% Read DATA
wholedata=xlsread('KHODRO.xlsx','A1:F800');%read whole data
moving_parameter=25;
...
for Numberof_seasons=1:period  % Numberof_seasons counter 
 ...
 for season=1:NUMBEROF_SEASON          
   for week=1:(LEAGUE-2)
       ...
       for Leaguesize=1:LEAGUE
           ...
           for day=1:25
              ...
              if signal==0 % for compeleting X when we have no signal and we have no rule
                  ...
                  reward=0;% -(punishmnet * data_trade(day,6));
              end 
              ...
              if signal ==2
                 ...
                 if sell_done ==1
                    ...
                    for i=1:rpc
                        j=rp_counter(i,1);
                        rule_point(j,1,Leaguesize)=reward;
                    end
                    rpc=0;
                    rp_counter=zeros(1,1);
                 end
       ...
       champion=sortedfitness(i,1);
       fitness_champion=sortedfitness(i,2);
       nonchampion=sortedfitness(end-i+1,1);
       fitness_nonchampion=sortedfitness(end-i+1,2);
       
       % Save the Rules in RULE POOLS.
       [ RULE_POOL_GOOD,RULE_POOL_BAD,BURN,BDRN,SURN,SDRN,LBURN,LBDRN,LSURN,LSDRN,BUY_UP,BUY_DOWN,SELL_UP,SELL_DOWN] = RULE_SAVER( RULE_POOL_GOOD,RULE_POOL_BAD,RP,champion,nonchampion,fitness_champion,fitness_nonchampion,BURN,BDRN,SURN,SDRN,LBURN,LBDRN,LSURN,LSDRN,BUY_UP,BUY_DOWN,SELL_UP,SELL_DOWN,up,down,rule_point,repetition_numebr);
       end
       RP=zeros(5,2,1,1);
